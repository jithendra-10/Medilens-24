...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchWithRetry = async (url: string, options: any, retries = 3, backoff = 800) => {
      for (let i = 0; i < retries; i++) {
        try {
          ...
          if (res.status === 401) return res; // Don't retry auth errors
        } catch (e) {
    ...
    const fetchUser = async () => {
      try {
        ...
        const res = await fetchWithRetry(`${apiUrl}/auth/me`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (res?.ok) {
          const userData = await res.json();
          setUser({ name: userData.full_name || userData.email, email: userData.email });
        }
      } catch (e) {
    ...
    // Profiles might still be local or handled by backend, but for now align with backend if available
    const fetchProfiles = async () => {
      try {
        const token = await getToken();
        const res = await fetchWithRetry(`${apiUrl}/profiles`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (res?.ok) {
    ...
    const fetchPreviousReports = async () => {
      try {
        const token = await getToken();
        const res = await fetchWithRetry(`${apiUrl}/dashboard/reports`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (res?.ok) {
          const reports = await res.json();
          // Map FastAPI ReportResponse back to what UI expects
          const mappedReports = reports.map((r: any) => {
            const structured = r.structured_data_json ? JSON.parse(r.structured_data_json) : {};
            return {
              id: r.id,
              patientName: structured.patient_info?.["Patient Name"] || "Patient",
              reportDate: structured.patient_info?.["Report Date"] || r.created_at,
              overallRisk: r.risk,
              fullPayload: r.structured_data_json // This will be parsed in the UI
            };
          });

          setPastReports(mappedReports);

          // If DB has no reports, clear any stale localStorage session so the
          // dashboard doesn't show a ghost card from a previously deleted report
          if (mappedReports.length === 0) {
            localStorage.removeItem("medilens_result");
            sessionStorage.removeItem("medilens_result");
            setData(null);
          } else {
            const currentSession = localStorage.getItem("medilens_result") || sessionStorage.getItem("medilens_result");
            if (!currentSession && mappedReports.length > 0) {
  ...
  if (!isLoaded) return <div className="min-h-[50vh] flex items-center justify-center"><div className="spinner text-primary" /></div>;

  const hasData = !!data;
  const analytics = data?.analytics || { total: 0, abnormal: 0 };
  const overallRisk = data?.alert?.alert_level || "Active";
      ...
      <section className="relative overflow-hidden liquid-glass p-12 rounded-[3rem] border-white/60 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8 group">
        ...
        <div className="relative z-10 space-y-4 max-w-xl">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
            Welcome back, <br />
            <span className="text-primary italic">{user?.name ? user.name.split(" ")[0] : "Friend"}</span>!
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {pastReports.slice(1, 4).map((report) => (
              <div key={report.id} className="liquid-glass p-6 rounded-[2rem] border-white/40 shadow-lg hover:shadow-2xl transition-all group relative overflow-hidden">
                ...
                <Button
                  onClick={() => {
                    localStorage.setItem("medilens_result", report.fullPayload);
