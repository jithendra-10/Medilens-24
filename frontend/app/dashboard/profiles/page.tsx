        ...
        const fetchWithRetry = async (url: string, options: any, retries = 3, backoff = 800) => {
            for (let i = 0; i < retries; i++) {
                try {
                    ...
                    console.warn(`Profiles fetch attempt ${i + 1} status: ${res.status}`);
                } catch (e) {
                                    ...
                                    <div className="flex items-start justify-between mb-6">
                                        ...
                                        <div className="px-4 py-1.5 rounded-full bg-primary/5 border border-primary/10">
                                            <span className="text-[10px] font-bold text-primary uppercase tracking-widest">
                                                {profile.report_count?.reports || 0} {profile.report_count?.reports === 1 ? 'Report' : 'Reports'}
                                            </span>
                                        </div>
                                    </div>
