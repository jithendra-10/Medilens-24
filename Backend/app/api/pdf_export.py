...
---------------------
POST /api/generate-pdf        — Full detailed report (with trend & AI explanation).
POST /api/generate-summary-pdf — Simple patient-friendly single-report summary.
"""
...
from app.services.pdf_summary import generate_simple_summary_pdf
...
async def generate_pdf(req: PDFRequest):
    """Generate and return the full detailed PDF health report (with trend & AI explanation)."""
    try:
        data = req.model_dump()

        # If explanation is missing or just the fallback, invoke the LLM RAG engine
...
async def generate_summary_pdf(req: PDFRequest):
    """Generate a simple, clean patient-friendly single-report summary PDF."""
    try:
        data = req.model_dump()
        pdf_bytes = generate_simple_summary_pdf(data)
        return StreamingResponse(
            io.BytesIO(pdf_bytes),
            media_type="application/pdf",
            headers={"Content-Disposition": "attachment; filename=MediLens_Health_Summary.pdf"},
        )
    except Exception as e:
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Summary PDF generation error: {str(e)}")
