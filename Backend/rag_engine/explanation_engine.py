...
def compute_confidence_score(tests: dict, analytics: dict) -> dict:
    ...
    deduct += len(alert["critical_tests"]) * 5
...
def compute_trend_comparison(current_tests: dict, previous_tests: dict) -> dict:
    ...
    for test_name, curr_data in current_tests.items():
        ...
        trend_entry = {
            ...
            "direction"     : direction,
            "current_status": curr_data.get("status", "N/A"),
        }
        ...
        if change_pct is not None and change_pct != 0:
            ...
            summary_lines.append(
                ...
                f"({sign}{change_pct}% {direction}) [{curr_data.get('status', 'N/A')}]"
            )
        else:
            summary_lines.append(
                f"  - {test_name}: {curr_val} {unit} (no change) [{curr_data.get('status', 'N/A')}]"
            )
...
def generate_report_explanation(
    ...
    prompt = build_prompt(
        ...
        mode=mode,
        language=language,
        emergency_flag=alert["emergency_flag"],
        confidence_score=confidence["percentage"],
        trend_summary=trend["summary"] if trend else None,
    )
